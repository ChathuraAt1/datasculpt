...
const stages = [
  { label: 'INGEST', title: 'Raw sources', detail: 'APIs · streams · lakes', icon: Database, color: 'text-brand-200', line: 'from-brand-300/70' },
  { label: 'SCULPT', title: 'AI transforms', detail: 'RAPIDS · cuDF · CUDA', icon: Sparkles, color: 'text-brand-300', line: 'from-brand-400/70' },
  { label: 'GOVERN', title: 'Trusted output', detail: 'Quality · lineage · policy', icon: ShieldCheck, color: 'text-brand-400', line: 'from-brand-500/70' },
];
  ...
  return <section className="relative mx-auto max-w-7xl px-5 pb-24 pt-20 lg:px-8 lg:pb-32 lg:pt-28">
    <div className="grid items-center gap-16 lg:grid-cols-[0.92fr_1.08fr] lg:gap-10">
      <div>
        ...
        <div className="mt-10 flex flex-wrap gap-x-7 gap-y-3 font-mono text-[0.68rem] uppercase tracking-wider text-slate-500"><span className="flex items-center gap-2"><CheckCircle2 size={14} className="text-brand-300" />Built for scale</span><span className="flex items-center gap-2"><CheckCircle2 size={14} className="text-brand-300" />Governed by design</span></div>
      </div>
      <PipelineVisual />
    </div>
...
function PipelineVisual() {
  return <GlassCard className="relative overflow-hidden p-5 sm:p-7" >
    ...
    <div className="relative space-y-4">
      {stages.map((stage, index) => { const Icon = stage.icon; return <div key={stage.label} className="relative"><motion.div initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.35 + index * 0.16, duration: 0.45 }} className="relative z-10 flex items-center gap-4 rounded-xl border border-slate-800 bg-slate-950/80 p-4"><div className={`grid h-10 w-10 shrink-0 place-items-center rounded-lg bg-slate-900 ${stage.color}`}><Icon size={19} /></div><div className="min-w-0"><p className="font-mono text-[0.6rem] tracking-[0.18em] text-slate-500">{stage.label}</p><p className="mt-1 text-sm font-semibold text-slate-100">{stage.title}</p><p className="mt-1 truncate font-mono text-[0.68rem] text-slate-500">{stage.detail}</p></div><div className="ml-auto flex items-center gap-2 font-mono text-[0.62rem] text-brand-300"><span className="h-1.5 w-1.5 rounded-full bg-brand-300" />SYNCED</div></motion.div>{index < stages.length - 1 && <div className={`ml-9 h-4 w-px bg-gradient-to-b ${stage.line} to-transparent`} />}</div>; })}
    </div>
    <div className="mt-7 flex items-center justify-between border-t border-slate-800/80 pt-5"><span className="flex items-center gap-2 font-mono text-[0.65rem] text-slate-500"><GitBranch size={14} className="text-brand-300" />production / main</span><span className="flex items-center gap-2 font-mono text-[0.65rem] text-brand-300"><Zap size={13} />42,810 rows/s</span></div>
  </GlassCard>;
}

function Metric({ value, label }: { value: string; label: string }) {
  return <div className="bg-slate-950/90 px-4 py-5 sm:px-6"><p className="font-mono text-xl font-semibold tracking-tight text-brand-300 sm:text-2xl">{value}</p><p className="mt-1 text-[0.68rem] uppercase tracking-wider text-slate-500">{label}</p></div>;
}
