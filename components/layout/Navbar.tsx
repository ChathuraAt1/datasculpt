  ...
  return <header className="relative z-20 border-b border-slate-800/70 bg-slate-950/70 backdrop-blur-xl">
    <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
      ...
      <div className="hidden items-center gap-3 md:flex">
        ...
        {!loading && (isAuthenticated ? <><Button href="/account/" variant="ghost" className="px-3.5 py-2 text-xs">Account</Button><button onClick={() => void logout()} className="text-xs text-slate-500 transition hover:text-brand-300">Sign out</button></> : <Button href="/auth/login/" variant="ghost" className="px-3.5 py-2 text-xs">Sign in</Button>)}
