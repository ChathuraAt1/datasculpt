...
export function AIChatbot() {
  ...
  return <>
    <motion.button type="button" whileHover={reducedMotion ? undefined : { y: -3 }} whileTap={reducedMotion ? undefined : { scale: 0.96 }} onClick={() => setOpen((value) => !value)} className="fixed bottom-5 right-5 z-[55] grid h-14 w-14 place-items-center rounded-full border border-brand-300 bg-brand-500 text-slate-50 shadow-[0_10px_30px_rgba(154,101,0,0.28)] transition hover:bg-brand-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2 focus-visible:ring-offset-[#fffdf2]" aria-label={open ? 'Close DataSculpt AI assistant' : 'Open DataSculpt AI assistant'} aria-expanded={open} aria-controls="datasculpt-ai-chat"><span className="pointer-events-none absolute inset-[-5px] rounded-full border border-brand-300/60" aria-hidden="true"><motion.span animate={reducedMotion ? undefined : { scale: [1, 1.18, 1], opacity: [0.55, 0, 0.55] }} transition={reducedMotion ? undefined : { duration: 2.4, repeat: Infinity, ease: 'easeInOut' }} className="absolute inset-0 rounded-full border border-brand-200" /></span><ChatSignalMark reducedMotion={Boolean(reducedMotion)} /></motion.button>
    <AnimatePresence>
      {open && <motion.aside id="datasculpt-ai-chat" initial={{ opacity: 0, y: reducedMotion ? 0 : 18, scale: reducedMotion ? 1 : 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: reducedMotion ? 0 : 12, scale: reducedMotion ? 1 : 0.98 }} transition={{ duration: reducedMotion ? 0 : 0.2 }} className="fixed bottom-24 right-4 z-[56] flex h-[min(650px,70vh)] w-[min(92vw,390px)] flex-col overflow-hidden rounded-2xl border border-brand-200 bg-white shadow-[0_20px_70px_rgba(81,50,0,0.2)]" aria-label="DataSculpt Guide chat">
        <div className="flex items-center justify-between border-b border-brand-100 bg-brand-50 px-4 py-3"><div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl border border-brand-200 bg-white text-brand-700"><ChatSignalMark reducedMotion={Boolean(reducedMotion)} /></span><div><p className="text-sm font-semibold text-slate-900">DataSculpt Guide</p><p className="text-[0.68rem] text-slate-500">Product guidance, on demand</p></div></div><div className="flex items-center gap-1"><button type="button" onClick={clearConversation} className="rounded-lg p-2 text-slate-500 hover:bg-white hover:text-brand-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500" aria-label="Clear conversation"><Eraser size={16} /></button><button type="button" onClick={() => setOpen(false)} className="rounded-lg p-2 text-slate-500 hover:bg-white hover:text-brand-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500" aria-label="Close chat"><X size={17} /></button></div></div>
...
function ChatSignalMark({ reducedMotion }: { reducedMotion: boolean }) {
  return <motion.svg viewBox="0 0 28 24" width="24" height="22" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    <motion.path d="M3 16.5L8.5 11L13 14.5L20.5 6.5L25 9.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" initial={{ pathLength: 0, opacity: 0.4 }} animate={reducedMotion ? { pathLength: 1, opacity: 1 } : { pathLength: [0, 1, 1], opacity: [0.35, 1, 0.7] }} transition={reducedMotion ? { duration: 0 } : { duration: 2.2, repeat: Infinity, repeatDelay: 0.45, ease: 'easeInOut' }} />
    <motion.circle cx="3" cy="16.5" r="2" fill="currentColor" animate={reducedMotion ? undefined : { scale: [1, 1.35, 1] }} transition={reducedMotion ? undefined : { duration: 2.2, repeat: Infinity, repeatDelay: 0.45, ease: 'easeInOut' }} />
    <motion.circle cx="25" cy="9.5" r="2" fill="currentColor" animate={reducedMotion ? undefined : { scale: [1, 1.35, 1] }} transition={reducedMotion ? undefined : { duration: 2.2, repeat: Infinity, repeatDelay: 0.45, ease: 'easeInOut', delay: 0.25 }} />
  </motion.svg>;
}
