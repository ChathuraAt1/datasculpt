...
export function AIChatbot() {
  ...
  return <>
    <motion.button
      ...
      aria-label={open ? 'Close DataSculpt AI assistant' : 'Open DataSculpt AI assistant'}
      aria-expanded={open}
      aria-controls="datasculpt-ai-chat"
    >
      <span className="pointer-events-none absolute inset-[-5px] rounded-full border border-brand-300/60" aria-hidden="true">
        <motion.span
          animate={reducedMotion ? undefined : { scale: [1, 1.18, 1], opacity: [0.55, 0, 0.55] }}
      ...
      {open && <motion.aside id="datasculpt-ai-chat" initial={{ opacity: 0, y: reducedMotion ? 0 : 18, scale: reducedMotion ? 1 : 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: reducedMotion ? 0 : 12, scale: reducedMotion ? 1 : 0.98 }} transition={{ duration: reducedMotion ? 0 : 0.2 }} className="fixed bottom-24 right-4 z-[56] flex h-[min(650px,70vh)] w-[min(92vw,390px)] flex-col overflow-hidden rounded-2xl border border-brand-200 bg-white shadow-[0_20px_70px_rgba(81,50,0,0.2)]" aria-label="DataSculpt Guide chat">
        <div className="flex items-center justify-between border-b border-brand-100 bg-brand-50 px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-slate-900">DataSculpt Guide</p>
              <p className="text-[0.68rem] text-slate-500">Product guidance, on demand</p>
            </div>
          </div>
          <div className="flex items-center gap-1">
            <button type="button" onClick={clearConversation} className="rounded-lg p-2 text-slate-500 hover:bg-white hover:text-brand-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500" aria-label="Clear conversation">
              <Eraser size={16} />
            </button>
            <button type="button" onClick={() => setOpen(false)} className="rounded-lg p-2 text-slate-500 hover:bg-white hover:text-brand-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500" aria-label="Close chat">
              <X size={17} />
            </button>
          </div>
        </div>
        <div className="flex-1 space-y-3 overflow-y-auto bg-[#fbfaf5] p-4" aria-live="polite">
          {messages.map((message) => (
            <div key={message.id} className={`flex gap-2.5 ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              ...
              <div className={`max-w-[85%] rounded-2xl px-3.5 py-3 text-sm leading-6 ${message.role === 'user' ? 'bg-brand-500 text-slate-50' : 'border border-brand-100 bg-white text-slate-700'}`}>
                {message.content}
              </div>
            </div>
          ))}
          ...
          {error && <p className="rounded-xl border border-brand-200 bg-brand-50 p-3 text-xs leading-5 text-brand-800" role="alert">{error}</p>}
          <div ref={endRef} />
        </div>
