      ...
      <div className="grid w-full max-w-5xl gap-8 lg:grid-cols-[0.8fr_1fr] lg:items-center">
        <div className="hidden lg:block">
          ...
          <p className="mt-6 max-w-md text-base leading-7 text-slate-400">
            Securely access your enterprise data engineering workspace,
            subscription controls, and platform workflows.
          </p>
        </div>
...
export function AuthDivider() {
  return (
    <div className="my-6 flex items-center gap-3 text-[0.65rem] font-mono uppercase tracking-[0.18em] text-slate-600">
  ...
  return (
    <label className="block text-sm text-slate-300">
      ...
      <input
        ...
        className="mt-2 w-full rounded-lg border border-slate-800 bg-slate-950/80 px-3.5 py-3 text-sm text-white outline-none transition placeholder:text-slate-600 focus:border-brand-400 focus:ring-1 focus:ring-brand-400"
      />
    </label>
  );
}
export function AuthMessage({
  ...
  return (
    <div
      ...
      className={`rounded-lg border p-3 text-sm leading-6 ${error ? "border-brand-500/40 bg-brand-950/30 text-brand-200" : "border-brand-500/30 bg-brand-950/30 text-brand-300"}`}
    >
      {children}
    </div>
  );
}
export function AuthLinks({ signup = false }: { signup?: boolean }) {
  return (
    <p className="mt-6 text-center text-sm text-slate-500">
      {signup ? "Already have an account?" : "New to DataSculpt?"}{" "}
      <Link
