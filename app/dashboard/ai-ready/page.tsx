...
export default function AiReadyWorkspacePage() {
  ...
  async function generate(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setOutput(renderProviderResult(envelope.data));
    } catch (requestError) {
...
function renderProviderResult(value: unknown): string {
  ...
  if (value && typeof value === 'object') {
    ...
    if (first && typeof first === 'object') {
      ...
      if (choice.message && typeof choice.message === 'object' && typeof (choice.message as Record<string, unknown>).content === 'string') return String((choice.message as Record<string, unknown>).content);
    }
  }
  try { return JSON.stringify(value, null, 2); } catch { return 'The service returned a result that could not be displayed.'; }
}
