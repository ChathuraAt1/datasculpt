...
const planOrder = ['developer', 'professional', 'scale', 'enterprise'];
...
export default function PricingPage() {
  ...
  async function loadPlans() {
    ...
    try {
      ...
      const active = Array.isArray(data) ? data.map(normalizePlan).filter((plan): plan is Plan => plan !== null && plan.isActive) : [];
      const ordered = planOrder.map((slug) => active.find((plan) => plan.slug === slug)).filter((plan): plan is Plan => Boolean(plan));
      if (!ordered.length) throw new Error('No active plans returned');
      setPlans(ordered); setPricingState('success');
